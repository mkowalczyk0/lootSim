...
| 2026-09-08 | PM review finding fixed | login paid scrypt only for names that exist, so response *time* could enumerate accounts even though the body couldn't; `verifyPassword` now always runs against the stored hash or a per-boot decoy hash of the same shape. |
| 2026-09-08 | **9f's screen integrated** | `feature/login-screen` (2d5c380) merged: real `src/ui/login.ts` (tabs, real `<form>`, no re-render on keystroke), `#login` in `index.html`, styles, Settings **Log out** row with `onLogout`; `main.ts` wires it to `account.logout()` + reload. Their placeholder `account.ts` dropped for mine. `npm test` 1281 checks green. Nothing click-tested by anyone — no browser in any session. |
| — | Was waiting on | lootsim-9f: `src/ui/login.ts`, `#login` in `index.html` + styles, the Settings **Log out** row + `onLogout` callback. `main.ts` wiring for `onLogout` is mine once their row exists. |
