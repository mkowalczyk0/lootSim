...
| The floor itself | seed + `RunConfigWire` → `generateLevel` | `configFromWire`; smoke checks a full level fingerprint for every mode (since MP-1) |
...
| 2026-09-08 | **MP-2 landed** — B1, B2, B3, B4, B5 | B1: `Dungeon.netLerp` + `advanceRemote` — a client slides enemies and allies toward the latest snapshot position over `LERP_SPAN` (1.2 snapshot intervals), extrapolates projectiles by the `vx, vy` now on the wire, and counts wind-ups / telegraph fills / hit flashes down between snapshots. B2: the movement half of `updateHero` is now `Dungeon.moveHero(hero, move, dash, dt, live)`, shared by the host and by the client's `predictLocal` / `predictStep`; the `in` packet carries `seq`, the host's `NetInput` queues whole packets and consumes one per tick recording `lastSeq`, `HeroSnap` echoes it as `ack` plus `dc`/`vx`/`vy`; the client's `InputLog` replays everything newer than the ack from the host's state. Dash is predicted. Measured in smoke §10 with a real 8-tick-RTT delay queue: median correction 0.02 px, worst 0.04 px, against an 11 px lead (the old blend tugged by a quarter of that every snapshot). B3: `HeroSnap.st` status bits, mirrored into every hero's `sc` on the client (prediction reads slows/roots off it). B4: `NetInput` centres the stick after 12 host ticks without a packet; a dropped packet's buttons fold into the next. B5: `in.ap` aim point → `NetInput.aimPoint`. Smoke: sections 9–10 of `=== multiplayer ===`. Solo sim unchanged (campaign numbers identical to baseline). |
| 2026-09-08 | **MP-3 landed** — C1, C2, C3, C7; `PROTOCOL_VERSION` → 2 | C1: `Snapshot.m` (minions, matched by id and slid like monsters, visual timers ticked on the client) and `Snapshot.c` (corpses, rebuilt). C2: the enemy row gains an affix-count + `MONSTER_AFFIXES` index tail; a client rebuilds `affixes` for the ring/glyphs and prefixes the name the way the host does. C3: `publish` no longer forwards a remote hero's own `levelUp` (its browser raises one from `got xp`). C7: `ul`/`ut`/`bt` dropped from `HeroSnap`; `dc`/`vx`/`vy`/`ack` and `plan.running`/`end.early` made required; version bumped to 2 so a stale tab is refused by the relay rather than misreading the stream. Busiest smoke snapshot 2.2 kB / 26 monsters ≈ 45 kB/s per player. C8 (host-side stats for remote heroes) deliberately left. Not done in this cluster: D4 beyond the notice, D5 hub-mate smoothing, D6 pause hint. |
| 2026-09-08 | **MP-4 landed** — D1 (owner-approved): the host picks any portal | The separate Party Portal is gone. In a room the host walks into the Delve, a rift or the Reliquary Portal and confirms it in that station's screen; `main.ts`'s `launchOrPlan` turns that confirm into `Party.setPlan(config, station)` instead of a dive, and `plan` carries a `RunConfigWire` + `station`. `Hub.partyTarget` / `partyStation` / `inPartyPortal` make *that* station the ready spot for everybody; a planet plan grows the expedition portal on every deck via `syncHub` and it goes again when the plan changes. Clients pressing confirm at a run station are told what the host picked; Comms/Quartermaster/Forge stay open to all. The run starts as `{ ...plan.config, players: size }`. A rift's last floor in a party ends with `endRun("extract")` and every player's own save opens its next tier. Hub renderer: pulsing cyan ring on the chosen portal + a one-line status. Comms Relay: the depth row is now a plan row (no left/right). Smoke: hub section + the D1 block at the end of §7. **Rifts and planets are now co-op modes**; CLAUDE.md's "delve only" and "Challenger stays solo" lines are the PM's to rewrite (D7 ruling). |
| — | Left open, by decision or scope | D3 reconnect (out of scope), D4 beyond the notice, D5 hub-mate smoothing, D6 pause hint, C8 host-side stats for remote heroes. Nothing here has been verified in a browser — every check is headless; the owner's 2-tab look on merge is the missing step. |
