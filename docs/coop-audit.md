...
| — | **Next: MP-4 (D1, owner-approved)** — host walks into any portal (delve / rift / planet), confirms, that station becomes the ready spot; `plan` carries a `RunConfigWire`; Comms Relay loses its depth row; `main.ts:169` station block relaxed for the host. | pending |
