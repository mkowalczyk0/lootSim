...
| 2026-09-09 | Challenger picker + guaranteed-rarity scaling (branch `feat/challenger-reach`) | The Dive/Tower/Rifts/StarMap commit screens gained a Challenger row (writes through to the same `GameState.challengerTier` Settings already uses — no second stored tier). Separately, from `CHALLENGER_GUARANTEE_TIER` (Nightmare X, `src/data/challenger.ts`) the Vigil's clear cache also carries one item forced to `DAILY_GUARANTEED_RARITY` ("legendary"), stepped to "mythic" by `challengerGuaranteedRarity` and held there through every Death March tier — the same mechanism the Convergence's `WEEKLY_GUARANTEED_RARITY` already had, now shared rather than reinvented, and now itself Challenger-scaled too. **Capped at mythic on purpose — divine was asked for ("Death March X = guaranteed divine") but withheld as a conflict with `rarity.ts`'s "keep it absurd, divine/unspoken chest-only" rule: a *guaranteed* divine on a fixed weekly/daily cadence is a more reliable route to it than crafting is allowed to be anywhere else in the game. Flagged to the PM/owner rather than shipped — this is the row to edit if the ruling comes back to allow it.** `npm test` green. Not browser-verified — say so plainly; the owner should eyeball the new Challenger row on all four commit screens (mouse click + keyboard nav) and confirm the guarantee's tier threshold and cap read the way "Nightmare X" / "Death March X" were meant to. |
