...
| `feat/summon-sprite-seam` | `9e0925a` | `summon-sprites` | Complete, working tree clean. **GATE-GREEN, merge-ready** — verified independently by the PM, not taken on report: 5,309 lines, `ALL CHECKS PASSED`, zero anchored `^FAIL` lines, no writer holding `/tmp/summonart-test.log`. `summonart` walked 27 units clean. The hardening *is* committed, verified on disk: the total decode (`SUMMON_UNITS[i] ?? "__unknown_unit__"` in `src/net/sync.ts`, falling through to the triangle) and the sorted-table assertion in `tools/summonart.ts` are both folded into `ee75b9f`. The facing convention and the `SUMMON_ELEMENT_WASH = 0.3` placeholder are documented in `docs/summon-sprite-seam.md`. |
