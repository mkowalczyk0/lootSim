...
| 2026-09-09 | Guaranteed reward now Challenger-scaled (branch `feat/challenger-reach`) | `WEEKLY_GUARANTEED_RARITY` ("legendary") is unchanged as the base, but the forced clear-cache item now reads `challengerGuaranteedRarity(WEEKLY_GUARANTEED_RARITY, challengerTier)` (`src/data/challenger.ts`): unchanged through Nightmare IX, one step to "mythic" at Nightmare X (`CHALLENGER_GUARANTEE_TIER`), held flat through every Death March tier. The Vigil got the identical mechanism newly added (it had none before — only a guaranteed key tier), gated the same way so Challenger-off payout is untouched. **Divine was asked for at Death March X and deliberately not built** — see the same note in `docs/daily-dungeon.md`'s ledger; a guaranteed weekly/daily divine conflicts with `rarity.ts`'s "keep it absurd" rule for the same reason a guaranteed mythic doesn't (mythic is already reachable deterministically via crafting/Ascend; divine and unspoken are not, by design). Flagged to the PM/owner as a call to make explicitly. `npm test` green, including `npm run rewards`'s "the rarity ladder is untouched" check. Not browser-verified. |
