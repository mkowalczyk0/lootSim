...
export const ATLAS: Record<string, AtlasSprite> = {
  ...
  // **The art in `strike` is a RECOVERY, not a blow — read this before assuming it is done.**
  // It was generated by interpolating apex -> rest, and interpolation between two endpoints
  // can only produce poses BETWEEN them. A strike's defining pose is not between them: it is
  // the extension past the apex, which no two-point path can reach. Measured, the frame
  // nearest each release frame walks back down the wind-up (cast f12, f10, then f5 four
  // times) and never leaves the wind-up's own path by more than 17% of its span. So the boss
  // un-coils and settles rather than hitting anything.
  //
  // It ships anyway because a smooth 0.48s settle is strictly better than the instant snap
  // to idle it replaces, and because the seam under it is right. What it needs is a third,
  // AUTHORED impact pose so the path is apex -> impact -> rest; the silhouette ceiling
  // (the alpha box is already the full frame) is what makes that pose hard to get and is
  // the open question, not the plumbing. See docs/animation.md.
  //
  // A caution worth keeping: `npm run windup` reports this tag as the best-travelling
  // animation in the repo — the only one whose travel peaks on its last frame. It is, and it
  // still is not a strike. The metric measures whether a pose MOVES, not whether it reads as
  // a blow, and it cannot tell "unwinds to rest" from "strikes and recovers". Green numbers
  // did not mean the art was right; looking at it did.
  //
  // The first boss with a RELEASE as well as a wind-up. `cast` ends on the pose at the
  // instant the shape resolves; `strike` carries the body from there back to rest, and its
  // last frame IS idle frame 0 (measured 0.0% silhouette difference), so the hand-off back
  // to the idle loop has no snap in it. Frame 0, `w`, `h` and `worldScale` are all unchanged
  // — only `cols` grew, which is the whole point of several tags sharing one strip.
  //
  ...
  "boss.exiled-tyrant":       { id: "boss.exiled-tyrant",       w: 98,  h: 103, worldScale: 0.9738, feet: 0.03,
    anim: { cols: 14, tags: { idle: { from: 0, to: 4,  seconds: 0.25, loop: true },
