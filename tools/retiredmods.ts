 ...
 *
 * `npm run modkeys` — the broader "no mod key without a live simulation read" rule — lives
 * on `fix/wardpower-riptout` and is **not on master yet**, so it could not be reused here.
 * This gate deliberately does not reimplement it; the two are complementary and should sit
 * side by side once that branch lands.
 */
